        ...
        <div class="row justify-content-center mt-1">
          <div class="p-2 w-100">
            <div class="row">
              ...
              <div class="col-4">
                <select class="form-select" v-model="state.parameterId" name="inviteParameterName">
                  <option
                    :key="`eligibleParam ${invite.id} ${param.parameterId}`"
                    v-for="param in getEligibleParams(invite)"
                    :value="param.parameterId"
                  >
                    {{ `${param.avatarName} > ${param.parameterName}` }}
                  </option>
                </select>
              </div>
              <div class="col-2">
            ...
            <ul class="mt-2 list-group">
              <li
                :key="`inviteParam ${invite.id} ${param.parameterId}`"
                v-for="param in invite.parameters"
              ...
              >
                <div class="input-group">
                  <input
                    ...
                    type="text"
                    readonly
                    :value="`${param.avatarName} > ${param.parameterName}`"
                  />
                  <button
                    type="button"
                    class="btn btn-outline-danger"
                    @click="() => deleteParameter(invite, param.parameterId)"
                  >
                    Delete
                  </button>
                </div>
            ...
            <div class="mt-2 row">
              ...
              <div class="col-4">
                <select class="form-select" v-model="state.avatarId" name="inviteAvatarName">
                  <option
                    :key="`eligibleAvatar ${invite.id} ${avatar.avatarId}`"
                    v-for="avatar in getEligibleAvatarChanges(invite)"
            ...
            <ul class="mt-2 list-group">
              <li
                :key="`inviteAva ${invite.id} ${ava.avatarId}`"
                v-for="ava in invite.changeableAvatars"
                class="list-group-item"
              >
                <div class="input-group">
                  ...
                  <button
                    type="button"
                    class="btn btn-outline-danger"
                    @click="() => deleteAvatarChange(invite, ava.avatarId)"
                  >
                    Delete
                  </button>
                </div>
